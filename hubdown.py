...
def ray_casting(ecran, scale, OMplayer, zplayer, vecv, longueur, colormap, map):
    ...
    rot = np.array([[cos(demi_angle), -sin(demi_angle)],
                   [sin(demi_angle), cos(demi_angle)]])
    direction = np.dot(rot, direction)  # valeur de départ
    for ray_num in range(N):
        # step élevé ça va plus vite mais moins de détails.
        for distance in range(1, longueur, 5):
            x, y = OMplayer + direction * distance
            Nn, M = map.shape
            #il y aura du SCALE là de dans plus tard.
            # map et colormap sont pas dans le même sens map i->y j->x
            j, i = int(round(x/scale)), int(round(y/scale))
            ...
            if not(notinmap):  # très élégant
                ...
                if yscreen < y_defaut[ray_num]:
                    ...
                    # map et colormap sont pas dans le même sens colormap i->x j->y
        ...
        rot = np.array([[cos(delta_theta), -sin(delta_theta)],
...
class HubDown():
    def __init__(self, screen, joueur: Joueur):
        ...
        self.colormap = pg.transform.smoothscale(
            self.joueur.carte.bg, (600, 450))  # sous forme d'image
        # array là où on va aller chercher les couleurs.
        self.colormap = pg.surfarray.array3d(self.colormap)
        N, M, L = self.colormap.shape
        for i in range(N):
            for j in range(M):
                self.colormap[i, j] = np.array(
                    [self.colormap[i, j, 0], self.colormap[i, j, 1], self.colormap[i, j, 2]])

    def update(self):
        self.longueur = 1500  # On voit jusqu'à 1500 pix ecran c'est à dire m ?
        N = HDWIDTH  # nb de rayons
        screen_array = np.full((N, HDHEIGHT, 3), np.array([200, 200, 255]))
        ecran = ray_casting(screen_array, self.joueur.carte.scale, np.array(self.joueur.OM), np.array(
            self.joueur.z), np.array(self.joueur.vecv), self.longueur, self.colormap, self.joueur.carte.map)
        self.view3D = pg.surfarray.make_surface(ecran)

    def draw(self):
        pg.Surface.blit(self.screen, self.view3D, self.hub)
        ...
        pg.draw.line(self.screen, (128, 0, 128), OMplayer,
                     OMplayer+self.longueur*direction)
